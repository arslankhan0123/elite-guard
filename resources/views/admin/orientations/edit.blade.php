...
@section('title', 'Orientation Edit')

@section('breadcrumbTitle', 'Orientation Edit')
        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-header justify-content-between d-flex align-items-center">
                    <h4 class="card-title shine">Edit Orientation</h4>
                ...
                <div class="card-body">
                    <form action="{{ route('orientations.update', $orientation->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="row mt-3">
                            <div class="col-md-6 mb-3">
                                <label class="form-label">Orientation Type <span class="text-danger">*</span></label>
                                <input type="text" name="type" class="form-control" value="{{ old('type', $orientation->type) }}"
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="status" class="form-select" required>
                                    <option value="1" {{ old('status', $orientation->status) == '1' ? 'selected' : '' }}>Active</option>
                                    <option value="0" {{ old('status', $orientation->status) == '0' ? 'selected' : '' }}>Inactive</option>
                                </select>
                                @error('status')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                        ...
                        <div class="row mt-3">
                            <div class="col-md-12 mb-3">
                                ...
                                @if($orientation->document)
                                    <div class="mt-2">
                                        <a href="{{ $orientation->document }}" target="_blank" class="btn btn-sm btn-soft-info">
                                            <i class="mdi mdi-file-document-outline"></i> Current Document
                                        </a>
                                    </div>
                                @endif
